...
/* The opening sheet of an operations report, not a marketing landing page:
   everything here — header, Executive Dashboard, Project Context and the
   festival poster — is sized to sit inside one desktop viewport with no
          ...
          <dl className="grid grid-cols-2 gap-px border border-ink-25 bg-ink-25 sm:grid-cols-4 lg:grid-cols-8">
            {data.metrics.map((metric) => (
              <div key={metric.k} className="bg-paper px-3 py-2.5">
                ...
                <dd className="mt-1 font-display text-[1.05rem] leading-none font-bold">
                  {metric.v}
                </dd>
              </div>
            ))}
